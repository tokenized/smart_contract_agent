...
func Test_SingleContract_TokenTransferFee(t *testing.T) {
	ctx := logger.ContextWithLogger(context.Background(), true, true, "")
	store := storage.NewMockStorage()
	cacher := cacher.NewSimpleCache(store)
	key, _ := bitcoin.GenerateKey(bitcoin.MainNet)
	transferFeeLockingScript, _ := key.LockingScript()
	agent, test := agents.StartTestAgentWithCacherWithInstrumentTransferFeeUsingInstrument(ctx, t,
		store, cacher, 5000, transferFeeLockingScript)

	for i := 0; i < 100; i++ {
		senderCount := mathRand.Intn(10) + 1
		MockExecuteSingleContractTransferTokenTransferFeeRandom(t, ctx, agent, test, senderCount,
			false)
	}
...
func MockExecuteSingleContractTransferTokenTransferFeeRandom(t *testing.T, ctx context.Context,
	agent *agents.Agent, test *agents.TestData, senderCount int, useMulti bool) {

	transferFee := uint64(0)
	transferFeeLockingScriptSize := 0
	transferFeeInCurrentToken := false
	if test.Instrument.Creation.TransferFee != nil &&
		test.Instrument.Creation.TransferFee.Quantity > 0 {
		transferFee = test.Instrument.Creation.TransferFee.Quantity

		transferFeeInCurrentToken = test.Instrument.Creation.TransferFee.UseCurrentInstrument
	...
	minimumBalance := uint64(5000)
	if transferFeeInCurrentToken {
		minimumBalance += transferFee
	}
	for {
		var newBalances []*state.MockBalanceData
		if useMulti {
			signerCount := mathRand.Intn(5) + 1
			signerThreshold := mathRand.Intn(signerCount) + 1
			newBalances = state.MockBalancesMultiSig(ctx, &test.Caches.TestCaches, test.Contract,
				test.Instrument, signerThreshold, signerCount, senderCount)
		} else {
			newBalances = state.MockBalances(ctx, &test.Caches.TestCaches, test.Contract, test.Instrument,
				senderCount)
		}
		...
		if totalQuantity > minimumBalance {
			break
		}
	}
	...
	if transferFeeInCurrentToken {
		sentQuantity -= transferFee // leave extra sender quantity to cover transfer fee
	}
			...
			Instruments: Instruments{
				{
					...
					TransferFeeInCurrentToken:    transferFeeInCurrentToken,
				},
			},
		},
	}
