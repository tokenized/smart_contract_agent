...
func Test_Responder_Response(t *testing.T) {
	...
	caches.Caches.Transactions.Release(ctx, requestTxID)
...
func Test_Responder_Request(t *testing.T) {
	...
	requestCetx := channelsExpandedTx.ExpandedTxMessage(*requestEtx)

	script, err := client.Wrap(&requestCetx, writePeerChannel)
	if err != nil {
	...
	transaction, err := caches.Caches.Transactions.Get(ctx, requestTxID)
	if err != nil {
		t.Fatalf("Failed to add transaction : %s", err)
	}
...
func Test_Responder_AlreadyProcessed(t *testing.T) {
	...
	transaction, err := caches.Caches.Transactions.AddExpandedTx(ctx, requestEtx)
	if err != nil {
		t.Fatalf("Failed to add transaction : %s", err)
	}
